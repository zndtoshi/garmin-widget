...
- **`bodyBatteryTimeline`** / **`stressTimeline`**: intraday sorted/deduped points with values clamped to 0–100 and extrema-aware downsampled to max 192 real Garmin samples (first/last + spikes; no fabricated points). Duplicate timestamps prefer the last valid sample.
- **`lastActivity`**: most recent activity. `startTimeGMT` is the trusted UTC source and may be parsed from either a naive GMT string or an offset-bearing timestamp. `startTimeLocal` is ignored when GMT is absent, so local-only timestamps are never mislabeled with `Z`. Activities with no recognized public fields return `null`. Optional `heartRateTimeline` / `speedTimeline` (max 240 chronological points each) are derived from a transient `get_activity_details(..., maxpoly=0)` fetch when a usable activity ID exists; activity IDs, GPS/route polylines, and raw detail channels are never serialized. Speed uses ground-speed keys only (`directSpeed`, `directGroundSpeed`), units m/s, range `0..150`, with extrema-aware downsampling. Without a usable ID, summary activity still succeeds and the timelines are omitted. Call budgets become 16/10 with details, otherwise 15/9.

The snapshot persists the complete expanded payload including trend/timelines/activity, surviving repository reload and service restarts. The activity HR timeline extension is deployed and live-verified; the Android client has successfully cached 48 real timeline points from the live service.
